"""
    ...
    Version 1.7.1
    ... literally just taking 2.7.1 and swapping the excel file names to make it homogeneous distractors VS
    
    Changed the instructions so that they get 1 set of every trial they will see in the upcoming block
    (before in 2.7.1 they simply got 2 practice trials - a TP and a TA in random order. Because distractors were hetero
      I just used equal proportions instead of a trial       
      for each condition to save time.)
"""
...
def ColorCapInstructions ():
    ...
    event.mouseButtons = [0,0,0]
    fourTrialWarning.setAutoDraw(True)
    win.flip()
    while mouse.getPressed()[0] != 1:
        pass
    fourTrialWarning.setAutoDraw(False)
    win.flip()
    event.mouseButtons = [0,0,0]
    
    
    #### Begin Dynamic Instructions ####
    instrConds = ["RC23VC","RC23MC","AB24VC","AB24MC"]
    shuffle(instrConds)
...
#------Prepare to start Routine"Instructions"-------
    ...
    #### Begin Dynamic Instructions ####
    instrConds = ["RC23RD","RC23RO","AB24RD","AB24RO"]
    shuffle(instrConds)
    for trial in instrConds:
        response = [0,0]
        rt = -1
...
if random() < .5:
    ...
    VisualSearch(CAP_BLOCKS, 'VSwSFT 1.5 Cap Color Conditions.csv')
    ShapeCapInstructions()
    VisualSearch(CAP_BLOCKS, 'VSwSFT 1.5 Cap Shape Conditions.csv')
else:
    ShapeCapInstructions()
    VisualSearch(CAP_BLOCKS, 'VSwSFT 1.5 Cap Shape Conditions.csv')
    ColorCapInstructions()
...
ConjSearchInstructions()
VisualSearch(NUM_BLOCKS, 'VSwSFT 1.5 Conditions.csv')
